...
class Route {
    ...
    withExternalSecurity() {
        return new PreRequestHandler(new RestHandler(this, this.app), checkExternalSecurity);
    }
    ...
    bind(restHandler) {
        ...
        if (!result.alreadyExist) {
            ...
            this.bindToRouter(
                ...
                (req, res, next) => {
                    // if ((req.headers.version !== config.version)) {
                    //     res.set('Cache-Control', 'no-store');
                    //     res.set('Pragma', 'no-cache');
                    //     res.set('Expires', 0);
                    //     res.set('version', config.version);
                    //     res.set('access-control-expose-headers', 'version');
                    //     return next(new ApiError(410, 'Gone', 'Version not matched'));
                    // }
...
function checkExternalSecurity(req, res, next) {
    console.log("in security body:",req.body)
    console.log("in security headers:",req.headers)
    const token = req.body.token || req.query.token || req.headers.authorization.replace("Bearer ", "");

    if (token) {
        jwt.verify(token, config.jwt_external_secret, (err, decoded) => {
            if (err) {
                next(new ApiError(401, 'unauthorized', 'Failed to authenticate token.'));
            } else {
                req.decoded = decoded;
